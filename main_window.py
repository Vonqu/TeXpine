...
class SpineDataSender:
    ...
    def _calculate_stage_weighted_value(self, sensor_data, config, stage_name=None):
        """计算单个阶段的加权归一化值"""
        # 若未从事件文件加载，尝试直接从控制面板读取
        if not self.events_file_loaded:
            if self.control_panel is None:
                print("警告: control_panel未设置，使用默认值0.5")
                return 0.5
            return self._calculate_simple_weighted_value(sensor_data, config.get('weights', []), stage_name)
        
        original_values = config.get('original_values', [])
        target_values = config.get('target_values', [])
        weights = config.get('weights', [])
        
        # 如果事件文件中没有完整参数，则从控制面板兜底读取
        if not original_values or not target_values or not weights:
            try:
                controller = self._get_controller_for_stage_name(stage_name)
                if controller is None:
                    return 0.5
                weights = []
                original_values = []
                target_values = []
                max_len = min(len(sensor_data), len(getattr(controller, 'sensor_checkboxes', [])))
                for i in range(max_len):
                    checked = controller.sensor_checkboxes[i].isChecked()
                    weight_val = controller.weight_spinboxes[i].value() if i < len(controller.weight_spinboxes) else 0.0
                    weights.append(weight_val if checked else 0.0)
                    # 读取 OV/RBV
                    ov = controller.original_value_spins[i].value() if i < len(controller.original_value_spins) else 2500
                    rbv = controller.rotate_best_value_spins[i].value() if i < len(controller.rotate_best_value_spins) else 2500
                    original_values.append(ov)
                    target_values.append(rbv)
            except Exception as e:
        ...
        for i, sensor_val in enumerate(sensor_data):
            if i < len(weights) and i < len(original_values) and i < len(target_values):
                ...
                if weight == 0:
                    continue
                original_val = float(original_values[i])
                target_val = float(target_values[i])
    ...
    def _calculate_simple_weighted_value(self, sensor_data, weights, stage_name=None):
        ...
        try:
            ...
            if controller is None:
                # 退化为遍历找到第一个可用控制器
                for ctrl in [
                    getattr(self.control_panel, 'gray_rotation', None),
                    getattr(self.control_panel, 'blue_curvature', None),
                    getattr(self.control_panel, 'gray_tilt', None),
                    getattr(self.control_panel, 'green_tilt', None)
                ]:
                    if ctrl and hasattr(ctrl, 'sensor_checkboxes') and hasattr(ctrl, 'weight_spinboxes'):
                        controller = ctrl
                        break
            if controller is None:
                return 0.5
            ...
            for i, sensor_val in enumerate(sensor_data):
                if i < len(weights) and float(weights[i]) != 0.0:
                    weight = float(abs(weights[i]))
                    original_val = float(original_values[i]) if i < len(original_values) else 2500.0
                    target_val = float(target_values[i]) if i < len(target_values) else 2500.0
                    if abs(original_val - target_val) > 1e-6:
                        normalized = (sensor_val - target_val) / (original_val - target_val)
                        normalized = max(0.0, min(1.0, normalized))
                    else:
    ...
    def _get_controller_for_stage_name(self, stage_name):
        """根据阶段配置键名返回控制器对象（与 stage_configs 的键对应）"""
        try:
            if self.control_panel is None:
                return None
            mapping = {
                'gray_rotation': getattr(self.control_panel, 'gray_rotation', None),
                'blue_curvature': getattr(self.control_panel, 'blue_curvature', None),
                'gray_tilt': getattr(self.control_panel, 'gray_tilt', None),
                'green_tilt': getattr(self.control_panel, 'green_tilt', None),
                # 兼容可能的键名
                'blue_curvature_up': getattr(self.control_panel, 'blue_curvature_up', None),
                'blue_curvature_down': getattr(self.control_panel, 'blue_curvature_down', None),
            }
            return mapping.get(stage_name, None)
        except Exception:
            return None
...
class SensorMonitorMainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.stage_sensor_mapping = {
            ...
            'pelvis_rotation': {
                ...
                'original_event': '开始训练',
                'target_event': '完成阶段',
                'spine_types': ['C', 'S']  # 适用的脊柱类型
            },
            
            # 阶段2：脊柱曲率矫正（C型单段）
            'spine_curvature_single': {
                'name': '脊柱曲率矫正',
                'stage_label': '阶段2',
                'sensor_indices': list(range(self.control_panel.get_num_sensors())),
                'original_event': '开始矫正',
                'target_event': '矫正完成',
                'spine_types': ['C']  # 仅适用于C型
            },
            
            # 阶段2A：脊柱曲率矫正·胸段（S型上段）
            'spine_curvature_upper': {
                'name': '脊柱曲率矫正·胸段',
                'stage_label': '阶段2A',
                'sensor_indices': list(range(self.control_panel.get_num_sensors())),
                'original_event': '开始矫正(胸段)',
                'target_event': '矫正完成(胸段)',
                'spine_types': ['S']  # 仅适用于S型
            },
            
            # 阶段2B：脊柱曲率矫正·腰段（S型下段）
            'spine_curvature_lower': {
                'name': '脊柱曲率矫正·腰段',
                'stage_label': '阶段2B',
                'sensor_indices': list(range(self.control_panel.get_num_sensors())),
            ...
            'pelvis_tilt': {
                'name': '骨盆左右倾斜',
                'stage_label': '阶段3',  # C型为阶段3，S型为阶段3
                'sensor_indices': list(range(self.control_panel.get_num_sensors())),
                'original_event': '开始沉髋',
                'target_event': '沉髋完成',
                'spine_types': ['C', 'S']  # 适用于C型和S型
            },
            
            # 阶段4：肩部左右倾斜（C型和S型共用）
            'shoulder_tilt': {
                'name': '肩部左右倾斜',
                'stage_label': '阶段4',  # C型为阶段4，S型为阶段4
                'sensor_indices': list(range(self.control_panel.get_num_sensors())),
                'original_event': '开始沉肩',
                'target_event': '沉肩完成',
                'spine_types': ['C', 'S']  # 适用于C型和S型
        ...
        # 实时刷新与发送限频设置
        self._last_udp_send_time = 0.0
        self._udp_send_hz = 50  # UDP发送上限频率(Hz)
        self._ui_refresh_hz = 40  # 界面刷新频率(Hz)
        self._last_bt_poll_log_time = 0.0
    ...
    def _connect_signals(self):
        ...
        # 连接积木可视化Tab的传感器参数更新请求信号
        self.blocks_manager.get_tab_widget().sensor_params_update_requested.connect(
            self.handle_sensor_params_update_request
        )
    ...
    def on_spine_direction_changed(self, spine_direction):
        ...
        # 不再需要重复的阶段控制信号连接
    # 事件记录已经在blocks_tab.py中的_handle_event_button_click中完成
    
    def handle_sensor_params_update_request(self, event_name, stage_key, sensor_data):
        """处理传感器参数更新请求（来自blocks_tab的信号）"""
        try:
            print(f"\n=== 处理传感器参数更新请求 ===")
            print(f"事件名称: {event_name}")
            print(f"阶段标志符: {stage_key}")
            
            # 检查阶段映射
            if stage_key not in self.stage_sensor_mapping:
                print(f"未找到阶段标志符 {stage_key} 的映射配置")
                return False
            
            stage_mapping = self.stage_sensor_mapping[stage_key]
            
            # 验证当前脊柱类型是否适用于此阶段
            current_spine_type = self.app_state["scoliosis"]["type"]
            if current_spine_type not in stage_mapping.get('spine_types', []):
                print(f"当前脊柱类型 {current_spine_type} 不适用于阶段 {stage_mapping['name']}")
                return False
            ...
            print(f"目标阶段: {stage_mapping['name']} ({stage_mapping['stage_label']})")
            
            # 更新tab2的传感器参数设置
            success = self.update_tab2_sensor_parameters(stage_key, stage_mapping, action_type, sensor_data)
    ...
    def add_save_weights_to_events_functionality(self):
        ...
        try:
            # 获取所有阶段的传感器选择器
            stage_selectors = {
                'pelvis_rotation': self._get_sensor_selector_for_stage('pelvis_rotation'),
                'spine_curvature_single': self._get_sensor_selector_for_stage('spine_curvature_single'),
                'spine_curvature_upper': self._get_sensor_selector_for_stage('spine_curvature_upper'),
                'spine_curvature_lower': self._get_sensor_selector_for_stage('spine_curvature_lower'),
                'pelvis_tilt': self._get_sensor_selector_for_stage('pelvis_tilt'),
                'shoulder_tilt': self._get_sensor_selector_for_stage('shoulder_tilt')
            }
    ...
    def _get_sensor_selector_for_stage(self, stage_key):
        ...
        try:
            ...
            stage_selector_mapping = {
                'pelvis_rotation': 'gray_rotation',          # 阶段1: 骨盆前后翻转
                'spine_curvature_single': 'blue_curvature',  # 阶段2: 脊柱曲率矫正(C型单段)
                'spine_curvature_upper': 'blue_curvature_up', # 阶段2A: 脊柱曲率矫正(S型上段)
                'spine_curvature_lower': 'blue_curvature_down', # 阶段2B: 脊柱曲率矫正(S型下段)
                'pelvis_tilt': 'gray_tilt',                  # 阶段3: 骨盆左右倾斜
                'shoulder_tilt': 'green_tilt'                # 阶段4: 肩部左右倾斜
            }
            
            selector_name = stage_selector_mapping.get(stage_key)
            if not selector_name:
                print(f"未找到阶段标志符 {stage_key} 对应的传感器选择器")
                return None
            ...
            print(f"未找到传感器选择器: {selector_name}")
            return None
    ...
    def update_tab2_sensor_parameters(self, stage_key, stage_mapping, action_type, sensor_data):
        ...
        try:
            ...
            # 根据阶段名称找到对应的参数组
            # 优先使用control_panel，如果没有则使用sensor_params
            search_target = getattr(blocks_tab, 'control_panel', sensor_params)
            parameter_group = self._find_parameter_group(search_target, stage_name)
    ...
    def _find_parameter_group(self, sensor_params, stage_name):
        ...
        try:
            # 直接映射阶段名称到控制面板属性
            stage_to_attr_mapping = {
                '骨盆前后翻转': 'gray_rotation',
                '脊柱曲率矫正': 'blue_curvature',
                '脊柱曲率矫正·胸段': 'blue_curvature_up',
                '脊柱曲率矫正·腰段': 'blue_curvature_down', 
                '骨盆左右倾斜': 'gray_tilt',
                '肩部左右倾斜': 'green_tilt'
            }
            
            # 首先尝试直接映射
            attr_name = stage_to_attr_mapping.get(stage_name)
            if attr_name:
                # 检查sensor_params是否是control_panel的包装
                control_panel = None
                if hasattr(sensor_params, 'control_panel'):
                    control_panel = sensor_params.control_panel
                elif hasattr(sensor_params, attr_name):
                    control_panel = sensor_params
                
                if control_panel and hasattr(control_panel, attr_name):
                    attr_value = getattr(control_panel, attr_name)
                    print(f"直接映射找到参数组: {stage_name} -> {attr_name}")
                    return attr_value
            ...
            # 检查sensor_params的结构
            search_targets = [sensor_params]
            if hasattr(sensor_params, 'control_panel'):
                search_targets.append(sensor_params.control_panel)
            
            for target in search_targets:
                # 方法1: 直接通过属性名查找
                for attr_name in stage_to_attr_mapping.values():
                    if hasattr(target, attr_name):
                        attr_value = getattr(target, attr_name)
                        if hasattr(attr_value, 'title') and attr_value.title() == stage_name:
                            print(f"通过属性名找到参数组: {stage_name} -> {attr_name}")
                            return attr_value
                
                # 方法2: 通过findChildren查找
                if hasattr(target, 'findChildren'):
                    all_groups = target.findChildren(QGroupBox)
                    for group in all_groups:
                        group_title = group.title() if hasattr(group, 'title') else ""
                        if group_title == stage_name:
                            print(f"通过title找到参数组: {stage_name} -> {group_title}")
                            return group
    ...
    def _update_spinboxes_in_group(self, group_widget, action_type, sensor_data, sensor_indices):
        """更新参数组中的SpinBox控件"""
        try:
            print(f"在参数组中查找SpinBox控件")
            print(f"参数组类型: {type(group_widget)}")
            print(f"传感器数据长度: {len(sensor_data)}")
            print(f"需要更新的传感器索引: {sensor_indices}")
            print(f"动作类型: {action_type}")
            
            # 直接通过SensorSelector的属性数组访问SpinBox
            target_spinboxes = None
            
            if action_type == 'original':
                # 查找原始值SpinBox数组
                if hasattr(group_widget, 'original_value_spins'):
                    target_spinboxes = group_widget.original_value_spins
                    print(f"    ✓ 找到original_value_spins数组，长度: {len(target_spinboxes)}")
            elif action_type == 'target':
                # 查找目标值SpinBox数组  
                if hasattr(group_widget, 'rotate_best_value_spins'):
                    target_spinboxes = group_widget.rotate_best_value_spins
                    print(f"    ✓ 找到rotate_best_value_spins数组，长度: {len(target_spinboxes)}")
            
            if not target_spinboxes:
                print(f"    ✗ 未找到{action_type}对应的SpinBox数组")
                return False
            ...
            for idx in sensor_indices:
                if idx >= len(sensor_data):
                    print(f"  跳过传感器索引 {idx}: 超出传感器数据范围")
                    skip_count += 1
                    continue
                    
                if idx >= len(target_spinboxes):
                    print(f"  跳过传感器索引 {idx}: 超出SpinBox数组范围")
                    skip_count += 1
                    continue
                
                sensor_value = sensor_data[idx]
                sensor_name = f"sensor{idx+1}"  # sensor1, sensor2, ...
                
                print(f"  处理传感器 {sensor_name} (索引{idx}): 值={sensor_value}")
                
                try:
                    spinbox = target_spinboxes[idx]
                    old_value = spinbox.value()
                    spinbox.setValue(int(sensor_value))
                    print(f"    ✓ 更新成功: {old_value} -> {int(sensor_value)}")
                    success_count += 1
                except Exception as e:
                    print(f"    ✗ 更新失败: {e}")
                    skip_count += 1
            
            print(f"更新结果: 成功 {success_count} 个，跳过 {skip_count} 个")
            return success_count > 0
            
        except Exception as e:
            print(f"更新SpinBox控件失败: {e}")
            import traceback
            traceback.print_exc()
            return False
                    ...
                    try:
                        ...
                        if callable(method):
                            ...
                            if data:
                                # 降低日志频率，最多每5秒打印一次
                                now = time.time()
                                if now - getattr(self, '_last_bt_poll_log_time', 0.0) >= 5.0:
                                    print(f"通过 {method_name} 获取到数据: {data[:5] if len(data) > 5 else data}")
                                    self._last_bt_poll_log_time = now
    ...
    def process_sensor_data(self, data):
        ...
        try:
            ...
            # 缓存用于界面刷新与患者端/积木更新（由定时器统一刷新）
            self._last_processed_data = processed_data
            
            # UDP发送（限频）
            if self.spine_data_sender.enable and len(processed_data) > 1:
                now = time.time()
                udp_interval = 1.0 / max(1, getattr(self, '_udp_send_hz', 50))
                if now - getattr(self, '_last_udp_send_time', 0.0) >= udp_interval:
                    current_mode = self.control_panel.get_current_mode()
    ...
    def _refresh_realtime_ui(self):
        """以固定频率刷新图表和相关界面，避免每个数据点都触发重绘"""
        try:
            ...
            if current_mode == "doctor":
                ...
                if self._last_processed_data is not None:
                    self.blocks_manager.process_sensor_data(self._last_processed_data)
            elif current_mode == "patient":
                self.plot_widget_tab1.update_plot(display_data)
                self.plot_widget_tab3.update_plot(display_data)
                if self.patient_blocks_tab and self._last_processed_data is not None:
                    self.patient_blocks_tab.update_sensor_data(self._last_processed_data)
        except Exception as e:
            print(f"实时界面刷新错误: {e}")
