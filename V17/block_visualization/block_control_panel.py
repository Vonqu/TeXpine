...
class BlockControlPanel(QWidget):
    ...
    def setup_ui(self):
        ...
        for control in [self.gray_rotation, self.blue_curvature, self.gray_tilt, self.green_tilt]:
            control.setFixedWidth(300)  # 增加宽度从250到300，避免文字被滚动条挡住
    ...
    def process_sensor_data(self, data_values):
        ...
        print(f"BlockControlPanel: 处理 {len(sensor_values)} 个传感器数据")
        ...
        for i, value in enumerate(sensor_values):
            if i < self.sensor_count:
                ...
                print(f"BlockControlPanel: 传感器索引 {i} 超出范围 (最大: {self.sensor_count-1})")
                break
    ...
    def set_sensor_count(self, count):
        ...
        # 更新所有传感器选择器的数量
        self.gray_rotation.set_sensor_count(count)
        self.blue_curvature.set_sensor_count(count)
        self.gray_tilt.set_sensor_count(count)
        self.green_tilt.set_sensor_count(count)
    ...
    def set_stage_defaults(self, stage, spine_type="C"):
        """根据阶段和脊柱类型设置OV和RBV的默认值"""
        if spine_type == "C":
            if stage == 1:
